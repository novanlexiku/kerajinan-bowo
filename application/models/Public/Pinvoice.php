...
class Pinvoice extends CI_Model {
	...
	{
		$query = $this->db->query("select * from tbl_invoice where code_inv = '$id' and handphone_inv = '$hp'");

		return $query->result();
	}

	// mengambil data aksesoris untuk invoice
	function load_inv($unique_id)
	{
		$this->db->select('*');
		$this->db->from('tbl_invoice');
		$this->db->join('tbl_bank', 'tbl_bank.id_bank = tbl_invoice.id_bank');
