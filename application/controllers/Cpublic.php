...
class Cpublic extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('public/paksesoris');
	...
	public function p_aksesoris()
	{
		...
		$var['aksesoris']			= $this->paksesoris->load_data();
	...
	public function p_aksesoriss($slug)
	{
		// var
		$var['title_web']		= $this->web_title();
		$var['page_web']		= $this->paksesoris->load_name($slug);

		// hootel data
		$var['aksesoris_data']		= $this->paksesoris->load_sc($slug);
	...
	public function p_process_form()
	{
		...
		$this->db->insert('tbl_invoice', $datainv);
		...
		$res = $this->paksesoris->load_inv($unique);

		foreach($res as $row)
		{
			$idaksesoris = $row->id_acc;
		}

		$var['aksesoris_data']		= $this->paksesoris->load_id($idaksesoris);
		$var['inv_data']		= $this->paksesoris->load_inv($unique);
		...
		$this->email->to($email);
		$this->email->subject('Pemesanan Aksesoris');
	...
	public function p_invoice($unique_id)
	{
		...
		$res = $this->paksesoris->load_inv($unique_id);
		...
		$var['aksesoris_data']		= $this->paksesoris->load_id($idaksesoris);
		$var['inv_data']		= $this->paksesoris->load_inv($unique_id);
		$this->load->view('public/template/head', $var);
		$this->load->view('public/template/menu_head', $var);
		$this->load->view('public/template/content', $var);
